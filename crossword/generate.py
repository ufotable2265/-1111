...
class CrosswordCreator():
                ...
                if neighbor not in assignment:
                    for neighbor_value in self.domains[neighbor]:
                        ...
                        if value[i] != neighbor_value[j]:
                            conflicts += 1
            return conflicts
        returnvar = sorted(self.domains[var], key=count_conflicts)
        return returnvar
    ...
    def select_unassigned_variable(self, assignment):
        ...
        # Degree heuristic (number of constraints)
        degree = max(unassigned_vars, key=lambda var: len(self.crossword.neighbors(var)))

        # If there's a tie, prefer the variable with the highest degree
